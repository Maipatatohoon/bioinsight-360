...
export function detectOutliers(librarySizes, detectionRates) {
  ...
  return librarySizes.map((size, idx) => {
    ...
    if (sizeZ > 3.0) {
      isOutlier = true;
      reason = `Extreme library size (Robust z=${sizeZ.toFixed(2)})`;
      status = 'Fail';
    } else if (rateZ > 3.0) {
      isOutlier = true;
      reason = `Low detection rate (Robust z=${rateZ.toFixed(2)})`;
      status = 'Fail';
    } else if (sizeZ > 2.0 || rateZ > 2.0) {
      reason = 'Moderate deviation from median';
      status = 'Warning';
    }
