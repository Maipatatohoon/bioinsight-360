...
export function runGOEnrichment(degList, goAnnotations, allGenes) {
  ...
  for (const go of goAnnotations) {
    ...
    // Restrict GO term genes to the background universe (case-insensitive)
    const validGenes = go.genes.filter(g => allSet.has(g.toUpperCase()));
    const K = validGenes.length;
    if (K === 0) continue;
    
    const overlappingGenes = validGenes.filter(g => degSet.has(g.toUpperCase()));
